...
```{r echo=T, results='hide', message=F, error=F, warning=F, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
```{r message=F, error=F, warning=F, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
```{r error=F, message=FALSE, warning=FALSE, include=FALSE, results='hide', tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
```{r echo=TRUE, error=F, message=FALSE, warning=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
```{r}
# Print the results
results <- bind_rows(results, tibble(Method = "Model 3: Average + Movie & User rating pattern bias", RMSE = model3_rmse))
results %>% 
  kbl(caption="MODEL PERFORMANCE RESULTS", digits=4) %>%
  kable_classic_2(full_width = T, latex_options = "hold_position")
...
```{r}
# Print the results
results <- bind_rows(results, tibble(Method = "Model 4: Average + Movie, User & Genres rating pattern bias", RMSE = model4_rmse))
results %>% 
  kbl(caption="MODEL PERFORMANCE RESULTS", digits=4) %>%
  kable_classic_2(full_width = T, latex_options = "hold_position")
...
results <- bind_rows(results, tibble(Method = "Model 5: Regularized Movie, User & Genres rating pattern bias", RMSE = model5_rmse))
results %>% 
  kbl(caption="MODEL PERFORMANCE RESULTS", digits=4) %>%
  kable_classic_2(full_width = T, latex_options = "hold_position")
...
```{r}
# Print the results
results <- bind_rows(results, tibble(Method = "Model 5: Matrix Factorization using recosystem", RMSE = model6_rmse))
results %>% 
  kbl(caption="MODEL PERFORMANCE RESULTS", digits=4) %>%
  kable_classic_2(full_width = T, latex_options = "hold_position")
...
```{r}
# Print the results
results <- bind_rows(results, tibble(Method = "Final Matrix Factorization - Recosystem Model with Hold-Out Validation Test Set", RMSE = final_model7_rmse))
results %>% 
  kbl(caption="MODEL PERFORMANCE RESULTS", digits=4) %>%
  kable_classic_2(full_width = T, latex_options = "hold_position")
...
# Conclusion
We started with a simple model and kept improving the model further until our model reached the target accuracy. Our best model uses the Recosystem package for Matrix Factorization with RMSE `r final_model7_rmse`, significantly less than our target of 0.8649.
